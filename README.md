...
- **Scan (Beta)** (`/scan`) — a viewport-locked camera view with a card-shaped guide; capture crops to the guide and **Azure gpt-4o-mini vision** reads the name (with a confidence), frame colour, attribute, type, level/ATK/DEF, passcode and set code. `lib/fuzzy.ts` then scores database candidates name → frame → attribute/type/stats, with the passcode as a bonus only, and auto-picks when the match is clear or offers a short pick list when it is not. Falls back to on-device OCR of the passcode when AI is off.
...
- **Import / Export** — the binder's Import button takes a CSV with just two required columns, `Name` and `Qty`; `Set Code`, `Rarity` and `Condition` are optional and pin the exact printing/condition when present. Repeated names add their quantities together, quantities are *set* (re-importing the same file is safe), and unknown names are reported. Export writes the full format.
