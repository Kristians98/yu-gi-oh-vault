...
`vercel.json` registers a daily cron to `GET /api/cron/refresh-prices`. Vercel sends
`Authorization: Bearer $CRON_SECRET`, which the route checks. Nothing else to do.
